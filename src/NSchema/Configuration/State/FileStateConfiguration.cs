using System.ComponentModel.DataAnnotations;
using NSchema.Configuration.Plugins;

namespace NSchema.Configuration.State;

/// <summary>
/// Configures a local-file state store.
/// </summary>
internal sealed class FileStateConfiguration
{
    private class FileOptions
    {
        [Required] public string Path { get; set; } = "";
    }

    /// <summary>
    /// The path to the state file.
    /// </summary>
    public string Path { get; set; } = "";

    /// <summary>
    /// Maps a <c>STATE file</c> statement's attributes onto a new config, rejecting any it doesn't recognize.
    /// </summary>
    /// <returns>The bound configuration, or a failure carrying the binder's own diagnostics.</returns>
    public static Result<FileStateConfiguration> FromSettings(PluginSettings config) => config.Get<FileStateConfiguration>();
}
