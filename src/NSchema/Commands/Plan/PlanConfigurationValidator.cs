...
internal sealed class PlanConfigurationValidator : AbstractValidator<PlanConfiguration>
{
    public PlanConfigurationValidator()
    {
        ...
        RuleFor(x => x.Database)
            .NotNull()
            .WithMessage("A database provider is required for plan: the plan's SQL is rendered against it. Declare a DATABASE statement.");
        ...
        RuleFor(x => x.State)
            .NotNull()
            .When(x => !x.Ephemeral)
            .WithMessage("A state store is required for plan: the plan diffs the recorded state. Declare a STATE statement, or pass --ephemeral.");
