...
public sealed class DiagnosticCatalogueTests
{
    ...
    private static List<(string Catalogue, string Member, string Source, string Code, string Severity, string Kind)> Catalogue()
    {
        ...
        // Only Core's catalogue — src/ also holds the CLI, which declares its own diagnostics.
        foreach (var file in Directory.EnumerateFiles(Path.Combine(SourceRoot(), "NSchema.Core"), "*Diagnostics.cs", SearchOption.AllDirectories))
        {
