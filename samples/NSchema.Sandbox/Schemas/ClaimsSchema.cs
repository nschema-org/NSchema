...
public class ClaimsSchema : AbstractSchemaProvider
{
    public ClaimsSchema()
    {
        ...
        // NOTE: The original SQL uses GENERATED ALWAYS AS IDENTITY (MINVALUE 0 START WITH 0).
        // NSchema supports IDENTITY columns but not custom MINVALUE/START options.
        events.Column("global_sequence", SqlType.BigInt).NotNull().Identity().Comment("Monotonically increasing sequence across all aggregates.");
        ...
        // NOTE: The original SQL uses a separate UNIQUE INDEX on global_sequence.
        // Represented here as a unique index; partial index (WHERE) syntax is not supported.
