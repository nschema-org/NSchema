...
public class IdentitySchema : AbstractSchemaProvider
{
    ...
    private static void AddProfileRoles(SchemaBuilder schema)
    {
        var table = schema.Table("profile_roles")
            ...
            .Grant(Roles.Api, TablePrivilege.All);
        table.Column("profile_id", SqlType.Text).NotNull().Comment("Foreign key referencing the profile.");
        table.Column("role_id", SqlType.Text).NotNull().Comment("Foreign key referencing the role.");
        table.PrimaryKey("profile_roles_pkey", ["profile_id", "role_id"]);
    ...
    private static void AddRolePermissions(SchemaBuilder schema)
    {
        var table = schema.Table("role_permissions")
            ...
            .Grant(Roles.Api, TablePrivilege.All);
        table.Column("role_id", SqlType.Text).NotNull().Comment("Foreign key referencing the role.");
        table.Column("permission_id", SqlType.Text).NotNull().Comment("Foreign key referencing the permission.");
        table.PrimaryKey("role_permissions_pkey", ["role_id", "permission_id"]);
    ...
    private static void AddAudit(SchemaBuilder schema)
    {
        ...
        table.Column("user_name", SqlType.Text).Comment("Name of the user.");
        table.Column("profile_id", SqlType.Text).Comment("Foreign key to the profile.");
        table.Column("profile_name", SqlType.Text).Comment("Name of the profile.");
        table.Column("role_id", SqlType.Text).Comment("Foreign key to the role.");
        table.Column("role_name", SqlType.Text).Comment("Name of the role.");
        table.Column("permission_id", SqlType.Text).Comment("Foreign key to the permission .");
        table.Column("permission_name", SqlType.Text).Comment("Name of the permission.");
